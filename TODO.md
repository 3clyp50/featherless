...
- [ ] **01:00** Register both URLs in a fresh PO workspace per `docs/po-registration.md`. MCP URL is `https://featherless-mcp.inf3ctious007.workers.dev/mcp`; AgentCard URL is `https://featherless-orchestrator.inf3ctious007.workers.dev/.well-known/agent-card.json`. Screenshot the workspace listing (MCP servers + External Agents pages) to `docs/sharp-proof/02-po-listing.png`.
