...
- [ ] **00:30** Deploy the orchestrator Worker (`npm run deploy:orchestrator`). Confirm `/.well-known/agent-card.json` is public and the `FEATHERLESS_MCP -> featherless-mcp` service binding reaches the MCP Worker.
...
- [ ] **01:00** Register both URLs in a fresh PO workspace per `docs/po-registration.md`. MCP URL is `https://featherless-mcp.inf3ctious007.workers.dev/mcp`; AgentCard URL comes from the orchestrator deploy. Screenshot the workspace listing (MCP servers + External Agents pages) to `docs/sharp-proof/02-po-listing.png`.
