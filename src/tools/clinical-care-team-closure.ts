...
export function buildCareTeamClosureResources(
  ...
  const labDue = addDays(ctx.encounter.date, 14);
  const checkInDue = addDays(ctx.encounter.date, 7);
  const eightWeekDue = addDays(ctx.encounter.date, 56);
  const generated = `${ctx.encounter.date}T12:00:00Z`;

  return [
    task(`task-${encounterId}-labs`, ctx, {
      description: "Schedule BMP + BNP draw",
      owner: "Lab scheduling",
      due: labDue,
    }),
    task(`task-${encounterId}-echo`, ctx, {
      description: "Schedule echocardiogram",
      owner: "Radiology scheduling",
      due: eightWeekDue,
    }),
    task(`task-${encounterId}-nurse-call`, ctx, {
      ...
      content: [
        {
          attachment: {
            ...
            title: `Patient packet for ${ctx.patient.name}`,
            data: base64Utf8(packetMarkdown),
          },
        },
      ],
      context: { encounter: [encounterRef(ctx)] },
    },
  ];
}
